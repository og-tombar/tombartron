...
class Polyhedron:
    def __init__(self, element_id: str = 'polyhedron', color: str = 'white', vertices: List[Vertex] = None,
...
class Triangle(Polyhedron):
    def __init__(self, element_id: str = 'triangle', color: str = 'white', vertices: List[Vertex] = None,
                 node: TransformNode = TransformNode()):
        super().__init__(element_id=element_id, color=color, vertices=vertices, node=node)
        self.default_vertices()
        self.normal = self.calc_normal()

    def default_vertices(self) -> None:
        top = Vertex(
            color=self.color, node=self.node, relative_pos_x=0, relative_pos_y=1, relative_pos_z=0)
        left = Vertex(
            color=self.color, node=self.node, relative_pos_x=-math.sqrt(3) / 2, relative_pos_y=-0.5, relative_pos_z=0)
        right = Vertex(
            color=self.color, node=self.node, relative_pos_x=math.sqrt(3) / 2, relative_pos_y=-0.5, relative_pos_z=0)
...
class Rectangle(Polyhedron):
    def __init__(self, element_id: str = 'rectangle', color: str = 'white', vertices: List[Vertex] = None,
                 node: TransformNode = TransformNode()):
        super().__init__(element_id=element_id, color=color, vertices=vertices, node=node)
        self.default_vertices()
    ...
    def default_vertices(self) -> None:
        bottom_left = Vertex(
            color=self.color, node=self.node, relative_pos_x=-0.5, relative_pos_y=-0.5, relative_pos_z=0)
        bottom_right = Vertex(
            color=self.color, node=self.node, relative_pos_x=0.5, relative_pos_y=-0.5, relative_pos_z=0)
        top_right = Vertex(
            color=self.color, node=self.node, relative_pos_x=0.5, relative_pos_y=0.5, relative_pos_z=0)
    ...
    def to_triangles(self) -> [Triangle]:
        t1 = Triangle(element_id='triangle1', vertices=[self.vertices[0], self.vertices[1], self.vertices[2]],
                      color=self.color, node=self.node)
...
class Cuboid(Polyhedron):
    def __init__(self, element_id: str = 'cuboid', color: str = 'white', vertices: List[Vertex] = None,
                 node: TransformNode = TransformNode()):
        super().__init__(element_id=element_id, color=color, vertices=vertices, node=node)
        self.node = node
        self.default_vertices()
    ...
    def default_vertices(self) -> None:
        front_bottom_left = Vertex(
            color=self.color, node=self.node, relative_pos_x=-0.5, relative_pos_y=-0.5, relative_pos_z=0.5)  # 0
        front_bottom_right = Vertex(
            color=self.color, node=self.node, relative_pos_x=0.5, relative_pos_y=-0.5, relative_pos_z=0.5)  # 1
        front_top_right = Vertex(
            color=self.color, node=self.node, relative_pos_x=0.5, relative_pos_y=0.5, relative_pos_z=0.5)  # 2
        front_top_left = Vertex(
            color=self.color, node=self.node, relative_pos_x=-0.5, relative_pos_y=0.5, relative_pos_z=0.5)  # 3

        back_top_left = Vertex(
            color=self.color, node=self.node, relative_pos_x=-0.5, relative_pos_y=0.5, relative_pos_z=-0.5)  # 4
        back_top_right = Vertex(
            color=self.color, node=self.node, relative_pos_x=0.5, relative_pos_y=0.5, relative_pos_z=-0.5)  # 5
        back_bottom_right = Vertex(
            color=self.color, node=self.node, relative_pos_x=0.5, relative_pos_y=-0.5, relative_pos_z=-0.5)  # 6
        back_bottom_left = Vertex(
            color=self.color, node=self.node, relative_pos_x=-0.5, relative_pos_y=-0.5, relative_pos_z=-0.5)  # 7
    ...
    def to_rectangles(self) -> [Rectangle]:
        ...
        front = Rectangle(element_id='front_side', color=self.color, vertices=[vtx[0], vtx[3], vtx[2], vtx[1]],
                          node=self.node)
        right = Rectangle(element_id='right_side', color=self.color, vertices=[vtx[1], vtx[2], vtx[5], vtx[6]],
                          node=self.node)
        back = Rectangle(element_id='back_side', color=self.color, vertices=[vtx[4], vtx[7], vtx[6], vtx[5]],
                         node=self.node)
        left = Rectangle(element_id='left_side', color=self.color, vertices=[vtx[0], vtx[7], vtx[4], vtx[3]],
                         node=self.node)
        bottom = Rectangle(element_id='bottom_side', color=self.color, vertices=[vtx[0], vtx[1], vtx[6], vtx[7]],
                           node=self.node)
        top = Rectangle(element_id='top_side', color=self.color, vertices=[vtx[2], vtx[3], vtx[4], vtx[5]],
                        node=self.node)
